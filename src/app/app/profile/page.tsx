...
// IMPORT HOOK BARU DISINI
import { useGetMe } from '@/app/api/Auth/services';
...
export default function ActivityPage() {
  // Hapus useUserData yang lama
  // const user = useUserData();

  const router = useRouter();

  // --- 0. FETCH DATA USER (REAL-TIME) ---
  const { data: userDataResponse, isLoading: isUserLoading } = useGetMe();

  // Ambil object user dari response API
  // Struktur: response.data.data (sesuai format CustomResponse backend kamu)
  ...
  // Handle Loading
  if (isWeeklyLoading || isMonthlyLoading || isCurrentLoading || isUserLoading)
    return (
    ...
    <section className="relative flex min-h-screen flex-col items-center p-4 pb-24 sm:p-5 md:h-full md:justify-center md:p-6">
      ...
      <div className="w-full max-w-2xl">
        ...
        <div className="flex flex-col items-center gap-2 py-4 sm:py-6">
          ...
          {/* NAMA USER YANG DINAMIS */}
          <h1 className="text-lg font-bold leading-7 sm:text-xl md:text-2xl">
            {/* Fallback ke 'Pengguna' jika data belum siap */}
            {user?.nama || 'Pengguna'}
          </h1>

          {/* Tampilkan Education Level jika ada */}
          {user?.educationLevel && (
